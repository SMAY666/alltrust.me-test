...
> При первом запуске создаётся тестовый мерчант с `feePercent = 0.03`. Его `merchantId` можно посмотреть в консоли или получить из MongoDB (коллекция `merchants`).
...
2. **Конкурентные webhook** — optimistic locking / версионирование документа для гонок при одновременных запросах.
3. **Валидация входных данных** — схемы (zod/joi) для `amount > 0`, допустимых валют, формата `merchantId`.
4. **API мерчантов** — CRUD для `feePercent`, получение `merchantId` без прямого доступа к БД.
5. **Логирование и метрики** — structured logs, trace id для webhook.
